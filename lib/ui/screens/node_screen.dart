...
/// Interface de chat dédiée NODE — IA de la station via NIP-44.
///
/// NODE (bro_dm_daemon.sh::_handle_bro) est le démon IA de la station
/// Astroport.ONE — distinct de BRO (canal self-DM, voir bro_screen.dart),
/// qui répond en tant qu'identité MULTIPASS de l'utilisateur, pas la station.
...
/// | #mem              | Afficher le slot 0 (personnel)                    |
...
/// | #reset            | Effacer uniquement le slot 0                      |
/// | #reset #N         | Effacer uniquement le slot N                      |
/// | #reset #all       | Effacer tous les slots (0-12)                     |
...
class _NodeScreenState extends State<NodeScreen> {
  ...
  bool _loading = true;
  bool _sending = false;
  bool _nodeTyping = false;
  String? _errorMsg;

  String? _myHexPubkey;
  String? _myHexPrivkey;
  String? _nodeHexPubkey;
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      final String? nodeHex = stationData?['NODEHEX'] as String?;
      if (nodeHex == null || nodeHex.length != 64) {
        _setError('Station NODE introuvable — vérifie ta connexion.');
        return;
      }
      final String? hostname = stationData?['hostname'] as String?;

      logger('[NodeScreen] NODEHEX → ${nodeHex.substring(0, 8)}… station=$hostname');

      if (mounted) {
        setState(() {
          _myHexPubkey = hexPub;
          _myHexPrivkey = hexPriv;
          _nodeHexPubkey = nodeHex;
      ...
      logger('[NodeScreen] init error: $e');
  ...
  Future<void> _loadHistory() async {
    if (_myHexPubkey == null || _myHexPrivkey == null || _nodeHexPubkey == null) {
      return;
    }
    ...
    try {
      ...
      logger('[NodeScreen] loadHistory error: $e');
  ...
  void _subscribeLive() {
    ...
    _dmSubId = NostrRelayService().subscribeToDms(
      ...
      onMessage: (NostrMessage msg) {
        if (!mounted) return;
        if (msg.senderHex != _nodeHexPubkey) return;
        // NODE a répondu → arrêter le typing indicator
        _typingTimer?.cancel();
        setState(() {
          _nodeTyping = false;
  ...
  Future<void> _send([String? override]) async {
    ...
    if (_myHexPrivkey == null || _nodeHexPubkey == null) return;
    ...
    // Timer de sécurité : NODE répond max en ~30s, 45s avant abandon indicator
    _typingTimer?.cancel();
    _typingTimer = Timer(const Duration(seconds: 45), () {
      if (mounted) setState(() => _nodeTyping = false);
    });

    final bool ok = await NostrRelayService().sendNip44Message(
      hexPrivateKey: _myHexPrivkey!,
      recipientHexPubkey: _nodeHexPubkey!,
      plaintext: text,
    );
  ...
  /// Demande confirmation avant d'envoyer #reset #all — un simple #reset
  /// sans argument n'efface plus que le slot 0 (voir table des commandes
  /// ci-dessus), ce bouton reste le seul chemin vers l'effacement total.
  Future<void> _confirmReset() async {
    final bool? confirmed = await showDialog<bool>(
      ...
      builder: (BuildContext ctx) => AlertDialog(
        ...
        content: const Text(
          'NODE effacera TOUS tes souvenirs (slots 0-12).\n'
    ...
    if (confirmed == true) await _send('#reset #all');
  }
  ...
  /// Bulle animée indiquant que NODE est en train de répondre.
  ...
  Widget _buildQuickCommands() {
    final bool canInteract = !_loading && !_sending && _nodeHexPubkey != null;
  ...
  void _showHelp() {
    showModalBottomSheet<void>(
      ...
      builder: (BuildContext ctx) => DraggableScrollableSheet(
        ...
        maxChildSize: 0.95,
        expand: false,
        builder: (_, ScrollController sc) => _NodeHelpSheet(
          scrollController: sc,
          stationHostname: _stationHostname,
          nodeHex: _nodeHexPubkey,
        ),
      ),
    );
  }
  ...
  Widget _buildBody(ColorScheme cs) {
    ...
    return Column(
      children: <Widget>[
        Expanded(
          child: _messages.isEmpty && !_nodeTyping
              ? Center(
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: <Widget>[
  ...
  Widget build(BuildContext context) {
    final ColorScheme cs = Theme.of(context).colorScheme;
    final bool ready = _nodeHexPubkey != null && !_loading;
...
class _NodeHelpSheet extends StatelessWidget {
  const _NodeHelpSheet({
      ...
      child: ListView(
        ...
        children: <Widget>[
            ...
            children: <Widget>[
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Text('NODE — Assistant IA de ta Station',
          ...
          _section(
            ...
            title: 'Qu\'est-ce que NODE ?',
            body:
                'NODE est le démon IA de ta station Astroport.ONE. Il traite tes '
          ...
          _section(
            ...
            body:
                'NODE dispose de 13 slots de mémoire personnelle :\n'
            ...
            Padding(
              ...
              child: Row(
                children: <Widget>[
                  Expanded(
                    child: Text(
                      'NODEHEX : ${nodeHex!.substring(0, 16)}…',
                      style: tt.bodySmall?.copyWith(
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      Clipboard.setData(ClipboardData(text: nodeHex!));
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
